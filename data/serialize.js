...
export default class Serialize {
  constructor(message, conn) {
    ...
    if (this.key) {
      ...
      this.isOwner = Array.isArray(config.owner)
        ? config.owner.some(owner => owner === this.sender) ||
          config.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(this.sender)
        : false;
      ...
      if (this.message) {
        ...
        this.command = this.isCmd ? this.body.slice(this.prefix.length).trim().split(/ +/).shift()?.toLowerCase() : undefined;
  ...
  getPrefix() {
    if (config.prefix.multiPrefix) {
      const match = this.body?.match(/^[°•π÷×¶∆£¢€¥®™✓_|~!?#$%^&.+-,\/\\©^]/gi);
      return match ? match[0] : config.prefix.singlePrefix;
    } else {
      return config.prefix.noPrefix ? '' : config.prefix.singlePrefix;
    }
  }
